<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="课程状态" width="109">
        <template slot-scope="scope">
          {{ scope.row.status==="1"?'已发布':'未发布' }}
        </template>
      </el-table-column>

      <el-table-column prop="lessonNum" width="160" label="课时数"/>

      <el-table-column prop="gmtCreate" label="添加时间" width="240"/>

      <el-table-column prop="viewCounr" label="浏览数量" width="200"/>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page=1) {
        this.page=page;
        course.getListCoursePage(this.page, this.limit,this.courseQuery).then(response => {
